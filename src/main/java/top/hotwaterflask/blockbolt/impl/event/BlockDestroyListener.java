...
public class BlockDestroyListener extends EventListener {
    ...
    public BlockDestroyListener(BlockBoltPluginImpl plugin) {
        super(plugin);
    }
    ...
    private Optional<Protection> willInterfereWithBreak(Player player, Block block) {
        // 1. Check if the block itself is protected by someone else
        Optional<Protection> selfProtection = getProtectionBySomeoneElse(player, block);
        if (selfProtection.isPresent()) {
            return selfProtection;
        }

        // 2. Check if breaking this block interferes with any of the 6 adjacent protected blocks (including supporting blocks)
        for (BlockFace searchFace : BlockFinder.NORTH_EAST_SOUTH_WEST_UP_DOWN) {
            Block nearBlock = block.getRelative(searchFace);
            Optional<Protection> nearProtection = getProtectionBySomeoneElse(player, nearBlock);
            if (nearProtection.isPresent()) {
                return nearProtection;
            }
        }
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        ...
        Optional<Protection> interferingProtection = willInterfereWithBreak(player, block);
        if (interferingProtection.isPresent()) {
            if (Permissions.has(player, Permissions.CAN_ADMIN)) {
                String ownerName = interferingProtection.get().getOwnerDisplayName();
                plugin.getTranslator().sendMessage(player, Translation.PROTECTION_BYPASSED, ownerName);
            } else {
                plugin.getTranslator().sendMessage(player, Translation.PROTECTION_NO_ACCESS, interferingProtection.get().getOwnerDisplayName());
        ...
        if (mainSign.isPresent()) {
            ...
            protection.ifPresent(p -> destroyOtherSigns(mainSign.get(), p));
        }
    }
    ...
    public void onBlockBurnEvent(BlockBurnEvent event) {
        ...
        if (isProtectedOrAdjacent(event.getBlock())) {
            event.setCancelled(true);
        }
    }
    ...
    public void onBlockExplodeEvent(BlockExplodeEvent event) {
        ...
        for (Iterator<Block> it = event.blockList().iterator(); it.hasNext();) {
            Block block = it.next();
            if (isProtectedOrAdjacent(block)) {
                it.remove();
            }
        }
    }
    ...
    public void onBlockPhysics(BlockPhysicsEvent event) {
        Block block = event.getBlock();
        Material material = block.getType();
        if (material.hasGravity() || material == Material.DRAGON_EGG) {
