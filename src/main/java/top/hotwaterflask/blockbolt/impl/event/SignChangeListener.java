...
public class SignChangeListener extends EventListener {
    ...
    public SignChangeListener(BlockBoltPluginImpl plugin) {
        super(plugin);
    }
    ...
    private void handleSignNearbyProtection(SignChangeEvent event, Protection protection) {
        ...
        if (!protection.isOwner(playerProfile) && !Permissions.has(player, Permissions.CAN_ADMIN)) {
        ...
        if (newSignType.filter(SignType::isMainSign).isPresent()) {
            ...
            if (oldSignType.isPresent()) {
                ...
                if (!Permissions.has(player, Permissions.CAN_ADMIN)) {
                    String ownerName = protection.getOwner().map(Profile::getDisplayName).orElse(player.getName());
                    if (!event.getLine(1).equals(ownerName)) {
                        if (!event.getLine(1).strip().equals(ownerName.strip())) {
