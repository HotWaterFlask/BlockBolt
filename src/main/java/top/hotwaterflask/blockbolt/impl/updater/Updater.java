...
import top.hotwaterflask.blockbolt.impl.updater.UpdateResult.Status;
...
public final class Updater {
    ...
    private final BlockBoltPluginImpl plugin;
    private volatile UpdatePreference preference;
    private final Translator translator;

    public Updater(UpdatePreference preference, Translator translator, BlockBoltPluginImpl plugin) {
    ...
    private UpdateNotifier activeNotifier;
    ...
    private void notifyServerFromServerThread(UpdateResult result) {
        ...
        if (activeNotifier != null) {
            org.bukkit.event.HandlerList.unregisterAll(activeNotifier);
        }
        activeNotifier = new UpdateNotifier(translator, result);

        // Notify players
        plugin.getServer().getPluginManager().registerEvents(activeNotifier, plugin);

        // Notify console
        activeNotifier.sendNotification(plugin.getServer().getConsoleSender());
    }
